...
uint64_t paging_k_pml4[512]__attribute__((aligned(0x1000)));

void paging_init(paging_ctx_t* ctx) {
    ctx->pml4 = paging_k_pml4;
...
paging_ctx_t* paging_create_context() {
    paging_ctx_t* ctx = (paging_ctx_t*)malloc(sizeof(paging_ctx_t));
    uintptr_t page = palloc_alloc(1);
    ctx->pml4 = (uint64_t*)page;
    paging_map(&paging_k_ctx, page, page, 1, PAGING_FLAG_RW);
    memset(ctx->pml4, 0, 0x1000);
    return ctx;
}
...
void paging_map(paging_ctx_t* ctx, uint64_t virt, uint64_t phy, size_t count, uint16_t flags) {
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        *pt_e = phy | PAGING_FLAG_P | flags;
        ...
        // If a mapping has been modifed, update tlb
        if (!new_path) {
